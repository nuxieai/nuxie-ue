...
namespace {
TSharedPtr<FJsonValue> Parse(const FNuxieJsonValue& Value) { TSharedPtr<FJsonValue> Result; FJsonSerializer::Deserialize(TJsonReaderFactory<>::Create(Value.ToJson()), Result); return Result; }
FNuxieJsonValue Encode(const TSharedRef<FJsonValue>& Value) { FString Json, Error; FJsonSerializer::Serialize(Value, TEXT(""), TJsonWriterFactory<>::Create(&Json)); FNuxieJsonValue Result; FNuxieJsonValue::TryParse(Json, Result, Error); return Result; }
}
...
bool UNuxieBlueprintLibrary::WithProperty(const FNuxieProperties& Properties, const FString& Name, const FNuxieJsonValue& Value, FNuxieProperties& Result, FString& Error) {
  auto Object = NuxieWire::Object(Properties.ToJson()); Object->SetField(Name, Parse(Value));
