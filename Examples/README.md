...
`BlueprintOnly/BlueprintOnly.uproject` has no Source directory or gameplay C++ module. Copy an output from `scripts/package.py` into its ignored `Plugins/Nuxie` directory. Open `Content/BP_NuxieGettingStarted`: its actual async graph reads project options, configures Nuxie, gets coherent identity, and branches on typed success/failure. Supply public keys in Project Settings → Plugins → Nuxie before mobile packaging. The prepared archive has been loaded and this graph compiled. Android packaging and execution of its unconfigured async error branch pass; iOS packaging passes, with physical launch qualification still pending.
