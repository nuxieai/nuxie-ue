      ...
      do {
        if method != "configure" {
          guard Self.owner === self, let session = self.session, arguments["session"] as? String == session else {
            throw failure("sessionExpired", "Configure Nuxie in this Unreal runtime before calling its API")
          }
        }
        ...
        case "configure":
          ...
          if sdk.isSetup && Self.configurationKey == nil { throw failure("alreadyConfigured", "Nuxie was configured outside Unreal") }
          Self.owner = self
        ...
        case "identify":
          ...
          resolve(try encode(["snapshot": self.snapshot(sdk.features.snapshot), "identity": identityDictionary()]))
        case "reset": sdk.reset(keepAnonymousId: false); resolve(try encode(["snapshot": self.snapshot(sdk.features.snapshot), "identity": identityDictionary()]))
...
private func failure(_ code: String, _ message: String) -> NSError {
  NSError(domain: "ai.nuxie.unreal", code: 1, userInfo: ["code": code, NSLocalizedDescriptionKey: message])
}
