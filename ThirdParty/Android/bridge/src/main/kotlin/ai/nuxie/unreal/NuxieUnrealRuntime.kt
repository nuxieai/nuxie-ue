...
class NuxieUnrealRuntime(activity: android.app.Activity, private val callback: Callback) {
  ...
  companion object {
    ...
    private var owner = WeakReference<NuxieUnrealRuntime>(null)
  ...
  fun invalidate() {
    scope.launch {
      ...
      session = null
      if (owner.get() === this@NuxieUnrealRuntime) { owner.clear(); if (Nuxie.listener === listener) Nuxie.listener = null }
      scope.cancel()
    }
  }
  ...
  private fun run(promise: Promise, attached: String? = null, block: suspend () -> Any?) {
    scope.launch {
      try {
        if (attached != null && (attached != session || owner.get() !== this@NuxieUnrealRuntime)) throw BridgeFailure("sessionExpired", "Configure Nuxie in this runtime first")
  ...
  private fun configure(configuration: String, promise: Promise) = run(promise) {
    ...
    if (Nuxie.isSetup && configurationKey == null) throw BridgeFailure("alreadyConfigured", "Nuxie was configured outside Unreal")
    ...
    try {
      val activity = activityReference.get()?.takeUnless { it.isDestroyed }
        ?: throw BridgeFailure("missingActivity", "The Unreal Activity is no longer available")
      ...
      if ((context.applicationInfo.flags and android.content.pm.ApplicationInfo.FLAG_DEBUGGABLE) != 0) {
        activity.intent?.getStringExtra("NUXIE_UNREAL_API_ENDPOINT")?.let {
      ...
      JSONObject(mapOf("session" to attached, "contract" to 1, "nativeVersion" to Nuxie.version,
        "snapshot" to Nuxie.features.snapshot.value.toWire(), "identity" to identityWire())).toString()
  ...
  private fun identify(session: String, customerId: String, properties: String, promise: Promise) = run(promise, session) {
    ...
    Nuxie.identify(customerId, options.optJSONObject("properties")?.toMap(), options.optJSONObject("propertiesSetOnce")?.toMap()); JSONObject(mapOf("snapshot" to Nuxie.features.snapshot.value.toWire(), "identity" to identityWire())).toString()
  }
  private fun reset(session: String, keepAnonymousId: Boolean, promise: Promise) = run(promise, session) { Nuxie.reset(false); JSONObject(mapOf("snapshot" to Nuxie.features.snapshot.value.toWire(), "identity" to identityWire())).toString() }
...
private fun identityWire() = mapOf("distinctId" to Nuxie.distinctId, "anonymousId" to Nuxie.anonymousId, "isIdentified" to Nuxie.isIdentified)
